...
class TransferMonitorController extends Controller
{
    public function index(Request $request)
    {
        if ($request->ajax()) {
            ...
            $query = StockTransfer::with(['fromStore', 'toStore', 'items'])
            ...
            return DataTables::of($query)
                ...
                ->addColumn('items_count', function($row) {
                    return $row->items->sum('quantity') . ' Units';
                })
    ...
    {
        $transfer = StockTransfer::with(['fromStore', 'toStore', 'items.product'])->findOrFail($id);
