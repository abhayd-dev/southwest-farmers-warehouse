...
namespace App\Http\Controllers\Warehouse\Finance;
...
class FinanceReportController extends Controller
{
    ...
    public function profitAndLoss(Request $request)
    {
        ...
        $trendQuery = \App\Models\Sale::selectRaw(\App\Support\Sql::dayKey('created_at') . " as date, SUM(total_amount - COALESCE(tax_amount, 0) - COALESCE(gst_amount, 0)) as revenue")
            ->whereBetween('created_at', [$startDate, $endDate])
            ->where('status', 'completed')
            ->groupBy('date')
            ->orderBy('date')
