                        ...
                        <div class="d-flex align-items-center gap-2 flex-wrap">
                            ...
                            <form method="GET" class="d-flex flex-grow-1" style="max-width: 500px;">
                                <div class="input-group shadow-sm">
                                    <input type="text" name="search" value="{{ request('search') }}" class="form-control border-end-0" placeholder="Search subcategories...">
                                    ...
                                    @if(request('search') || request('status') !== null)
                                        <a href="{{ route('warehouse.subcategories.index') }}" class="btn btn-outline-secondary"><i class="mdi mdi-close"></i></a>
                                    @endif
                                </div>
                            </form>
                            {{-- ACTIONS --}}
                            <a href="{{ route('warehouse.subcategories.export') }}" class="btn btn-outline-primary"><i class="mdi mdi-download"></i> Export</a>
                            <button type="button" class="btn btn-outline-success" data-bs-toggle="modal" data-bs-target="#importSubCategoryModal">
                                <i class="mdi mdi-upload"></i> Import
                            </button>
                            <a href="{{ route('warehouse.subcategories.create') }}" class="btn btn-success"><i class="mdi mdi-plus-circle"></i> Add Subcategory</a>
                        </div>
                    </div>
                </div>
            </div>
                    ...
                    <table class="table table-hover align-middle mb-0">
                        <thead class="bg-light">
                            <tr>
                                ...
                                <th class="py-3 text-muted fw-semibold small">SUBCATEGORY NAME</th>
                        ...
                        <tbody>
                            @forelse($subcategories as $sub)
                                <tr class="border-bottom">
                                    <td class="px-4 py-3 text-muted">{{ $loop->iteration + ($subcategories->currentPage()-1)*$subcategories->perPage() }}</td>
                                    <td class="py-3"><span class="badge bg-primary bg-opacity-10 text-primary">{{ $sub->category->name ?? 'N/A' }}</span></td>
                                    <td class="py-3 fw-semibold">{{ $sub->name }}</td>
                                    <td class="py-3"><code class="bg-light px-2 py-1 rounded">{{ $sub->code }}</code></td>
                                    <td class="py-3 text-center">
                                        <div class="form-check form-switch d-inline-block">
                                            <input class="form-check-input status-toggle" type="checkbox" data-id="{{ $sub->id }}" {{ $sub->is_active ? 'checked' : '' }}>
                                        </div>
                                    </td>
                                    <td class="px-4 py-3 text-end">
                                        <button class="btn btn-sm btn-outline-info me-1 view-btn" data-data="{{ json_encode($sub) }}" data-cat="{{ $sub->category->name ?? 'N/A' }}" title="View"><i class="mdi mdi-eye"></i></button>
                                        <a href="{{ route('warehouse.subcategories.edit', $sub) }}" class="btn btn-sm btn-outline-primary me-1" title="Edit"><i class="mdi mdi-pencil"></i></a>
                            ...
                            @empty
                                <tr><td colspan="6" class="text-center py-5 text-muted">No subcategories found.</td></tr>
                            @endforelse
                        </tbody>
                    </table>
                </div>
