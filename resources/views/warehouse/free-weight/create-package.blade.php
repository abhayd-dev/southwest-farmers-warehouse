                ...
                <div class="d-flex align-items-center gap-3 mb-4">
                    ...
                    <div>
                        ...
                        <small class="text-muted">
                            Define a package size for:
                            <strong>{{ $bulkProduct->product->product_name ?? 'N/A' }}</strong>
                        </small>
                    </div>
                </div>
                    ...
                    <div class="card-body p-4">
                        <form action="{{ route('warehouse.free-weight.package.store', $bulkProduct->id) }}"
                            ...
                            <div class="row g-3 mb-3">
                                <div class="col-8">
                                    <label class="form-label fw-semibold">Package Size <span
                                            class="text-danger">*</span></label>
                                    <input type="number" name="package_size" step="0.01" min="0.01"
                                        class="form-control" value="{{ old('package_size') }}" placeholder="e.g. 10"
                                        required>
                            ...
                            <div class="row g-3 mb-3">
                                <div class="col-6">
                                    ...
                                    <input type="text" name="upc" class="form-control"
                                        value="{{ old('upc') }}" placeholder="e.g. LENTIL-10LB" required>
                                </div>
                                <div class="col-6">
                                ...
                                <select name="target_product_id" class="form-select">
                                    ...
                                    @foreach ($products as $product)
                                        <option value="{{ $product->id }}"
                                            {{ old('target_product_id') == $product->id ? 'selected' : '' }}>
                                            {{ $product->product_name }} ({{ $product->upc ?? 'No UPC' }})
                                        </option>
                                    @endforeach
                                </select>
                                <div class="form-text">When packaging happens, stock for this product will be increased
                                    automatically.</div>
                            </div>
