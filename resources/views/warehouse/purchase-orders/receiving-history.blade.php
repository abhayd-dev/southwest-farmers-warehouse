<x-app-layout title="Receiving History — {{ $purchaseOrder->po_number }}">
    <div class="container-fluid p-3 p-md-4">
        ...
        <div
            ...
            <span
                class="badge bg-{{ $purchaseOrder->status === 'completed' ? 'success' : ($purchaseOrder->status === 'partial' ? 'warning' : 'secondary') }} fs-6 px-3 py-2">
            ...
            <div class="col-6 col-md-3">
                <div class="card border-0 shadow-sm text-center py-3">
                    <div class="fw-bold fs-3 text-warning">
                        {{ $purchaseOrder->items->sum('requested_quantity') - $purchaseOrder->items->sum('received_quantity') }}
                    </div>
                        ...
                        <table class="table table-hover align-middle mb-0">
                            ...
                            <tbody>
                                @foreach ($batches as $batch)
                                    ...
                                    @endphp
                                    <tr>
                                        <td class="px-4 py-3 fw-semibold font-monospace">{{ $batch->batch_number }}
                                        </td>
                                        <td class="py-3">
                                            <div class="fw-semibold">{{ $batch->product->product_name ?? 'N/A' }}</div>
                                            <small class="text-muted">{{ $batch->product->upc ?? '' }}</small>
                                        </td>
                                        <td class="py-3 text-center fw-bold">{{ number_format($batch->quantity, 0) }}
                                        </td>
                                        <td class="py-3">
