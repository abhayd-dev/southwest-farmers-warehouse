<x-app-layout title="Process Request #{{ $stockRequest->id }}">
            ...
            <div class="py-3">
                <div
                    ...
                    <div class="d-flex flex-wrap gap-2 w-100 w-md-auto justify-content-md-end">
                        <button class="btn btn-success shadow-sm flex-fill flex-md-grow-0" data-bs-toggle="modal"
                            data-bs-target="#stockInModal">
                            <i class="mdi mdi-plus-box me-1"></i> Stock In
                        </button>

                        <a href="{{ route('warehouse.stock-requests.index') }}"
                            class="btn btn-outline-secondary shadow-sm flex-fill flex-md-grow-0">
                ...
                <div class="card border-0 shadow-sm h-100 mb-4">
                    ...
                    <div class="card-body">
                        ...
                        <div class="row text-center g-0">
                            ...
                            <div class="col-4">
                                <small class="text-muted d-block mb-1 text-uppercase fw-bold"
                                    style="font-size: 0.7rem;">Pending</small>
                ...
                <div class="card border-0 shadow-sm bg-info bg-opacity-10 mb-4">
                    <div class="card-body">
                        ...
                        <div class="d-flex align-items-baseline mb-3">
                            <h2
                                class="fw-bold mb-0 {{ $totalStock < $stockRequest->pending_quantity ? 'text-danger' : 'text-dark' }}">
                                {{ $totalStock }}
                            </h2>
                            <small class="text-muted ms-2 fw-semibold">{{ $stockRequest->product->unit ?? 'Units' }}
                                available</small>
                        </div>

                        <div>
                            <small class="text-muted fw-bold text-uppercase" style="font-size: 0.7rem;">FIFO Batches
                                (Auto-Selected)</small>
                            <ul class="list-group list-group-flush bg-transparent mt-2">
                                @forelse($stockRequest->product->batches->take(3) as $batch)
                                    <li
                                        class="list-group-item bg-transparent d-flex justify-content-between px-0 py-2 border-bottom border-info border-opacity-25">
                                        <div>
                                            <span
                                                class="small fw-bold text-dark d-block">{{ $batch->batch_number }}</span>
                                            <span class="small text-muted" style="font-size: 0.7rem;">Exp:
                                                {{ $batch->expiry_date ? $batch->expiry_date->format('M Y') : 'N/A' }}</span>
                                        </div>
                                        <span
                                            class="badge bg-white text-info border border-info border-opacity-25 align-self-center">{{ $batch->quantity }}</span>
                                    </li>
                                @empty
                                    <li class="list-group-item bg-transparent text-danger small fst-italic px-0">No
                                        active batches found!</li>
                                @endforelse
                                @if ($stockRequest->product->batches->count() > 3)
                                    <li
                                        class="list-group-item bg-transparent text-center small text-muted py-2 fst-italic">
                        ...
                        @if ($stockRequest->pending_quantity > 0 && $stockRequest->status !== 'rejected')
                            ...
                            <form id="dispatchForm" class="needs-validation" novalidate method="POST">
                                ...
                                <div class="mb-4" id="dispatch_qty_div">
                                    <label class="form-label fw-bold">Quantity to Dispatch <span
                                    ...
                                    <div class="input-group input-group-lg has-validation">
                                        <input type="number" name="dispatch_quantity" id="dispatch_qty"
                                            ...
                                            min="1" required>
                                        <span
                                            class="input-group-text bg-light text-muted fw-bold">{{ $stockRequest->product->unit ?? 'Units' }}</span>
                                        <div class="invalid-feedback fw-bold" id="qty-error">
                                            Please enter a valid quantity (1 -
                                            {{ min($stockRequest->pending_quantity, $totalStock) }}).
                                        </div>
                                    </div>
                                    <div class="form-text text-muted mt-2 d-flex align-items-start gap-2">
                                        ...
                                        <span>
                                            ...
                                            Max dispatchable: <strong
                                                class="text-dark">{{ min($stockRequest->pending_quantity, $totalStock) }}</strong>
                                        </span>
                                    </div>
                                </div>

                                <div class="mb-4">
                                    <label class="form-label fw-bold">Admin Note <span
                                            class="text-muted fw-normal">(Optional)</span></label>
                                    <textarea name="admin_note" class="form-control" rows="3"
                                        placeholder="Add a message for the store manager..."></textarea>
                                </div>
                            ...
                            <div id="rejectSection"
                                ...
                                <form id="rejectForm" method="POST">
                                    ...
                                    <h6 class="text-danger fw-bold mb-3"><i
                                            class="mdi mdi-alert-circle-outline me-1"></i> Reject Request</h6>

                                    <div class="mb-3">
                                        <label class="form-label small fw-bold text-dark">Reason for Rejection <span
                                                class="text-danger">*</span></label>
                                        <textarea name="admin_note" class="form-control" rows="3" placeholder="Why are you rejecting this request?"
                                            required></textarea>
                                    </div>
                        ...
                        @else
                            ...
                            <div class="text-center py-5">
                                @if ($stockRequest->status == 'rejected')
                                    <div class="mb-3">
                                        <div
                                            class="avatar-lg mx-auto bg-danger bg-opacity-10 rounded-circle d-flex align-items-center justify-content-center">
                                            <i class="mdi mdi-close-circle text-danger display-4"></i>
                                        </div>
                                    </div>
                                    <h4 class="fw-bold text-danger">Request Rejected</h4>
                                    <p class="text-muted mx-auto" style="max-width: 400px;">Reason:
                                        {{ $stockRequest->admin_note ?? 'N/A' }}</p>
                                @elseif($stockRequest->status == 'dispatched')
                                    <div class="mb-3">
                                        <div
                                            class="avatar-lg mx-auto bg-info bg-opacity-10 rounded-circle d-flex align-items-center justify-content-center">
                                    ...
                                    <button class="btn btn-primary mt-3 px-4 shadow-sm"
                                ...
                                @else
                                    <div class="mb-3">
                                        <div
                                            class="avatar-lg mx-auto bg-success bg-opacity-10 rounded-circle d-flex align-items-center justify-content-center">
                                    ...
                                    <div class="mt-4 p-3 bg-light rounded text-start mx-auto border"
                                        style="max-width: 400px;">
                                        <h6 class="small fw-bold text-uppercase text-muted border-bottom pb-2 mb-2">
                                ...
                                @endif
                                <a href="{{ route('warehouse.stock-requests.index') }}"
                                    class="btn btn-dark mt-4 px-4 shadow-sm">
                    ...
                    <div class="card border-0 shadow-sm">
                        <div
                            ...
                            <h6 class="card-title mb-0 fw-bold text-dark">
                                <i class="mdi mdi-clipboard-list-outline text-primary me-1"></i> Dispatched Batch
                                Details (FIFO)
                            </h6>
                        </div>
                            ...
                            <div class="table-responsive">
                                <table class="table table-striped table-hover mb-0 align-middle">
                                    ...
                                    <tfoot class="bg-light">
                                        <tr>
                                            <td colspan="3"
                                                class="text-end fw-bold py-3 text-uppercase small text-muted">Total
                                                Dispatched:</td>
    ...
    <div class="modal fade" id="verifyModal" tabindex="-1">
        <div class="modal-dialog modal-dialog-centered">
            <form id="verifyForm" class="modal-content border-0 shadow" method="POST"
                ...
                <div class="modal-body p-4">
                    ...
                    <div class="mb-4">
                        ...
                        <div class="p-3 border rounded bg-light text-center">
                            ...
                            @else
                                <span class="text-danger small fw-bold"><i class="mdi mdi-alert-circle me-1"></i> No
                                    payment proof uploaded.</span>
                            @endif
                        </div>
                    </div>
                    ...
                    <div class="mb-3">
                        <label class="form-label fw-bold">Warehouse Payment Proof <span
                                class="text-danger">*</span></label>
                    ...
                    <div class="mb-3">
                        <label class="form-label fw-bold">Verification Remarks <span
                                class="text-danger">*</span></label>
                        <textarea name="warehouse_remarks" class="form-control" required rows="2"
                            placeholder="Enter transaction ID or notes..."></textarea>
                    </div>
                </div>
        ...
        <div class="modal-dialog modal-dialog-centered">
            <form id="stockInFormShow" class="modal-content border-0 shadow" method="POST">
                ...
                <div class="modal-header bg-success text-white">
                    <h5 class="modal-title fw-bold"><i class="mdi mdi-plus-box me-2"></i> Purchase In (Direct Stock)
                    </h5>
                    <button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal"></button>
                </div>
                <div class="modal-body p-4">
