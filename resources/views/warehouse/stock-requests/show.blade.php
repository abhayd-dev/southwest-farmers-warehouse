<x-app-layout title="Process Request #{{ $stockRequest->id }}">
            ...
            <div>
                <nav aria-label="breadcrumb">
                    <ol class="breadcrumb mb-1">
                        <li class="breadcrumb-item"><a href="{{ route('warehouse.stock-requests.index') }}">Requests</a>
                        </li>
        ...
        <div class="row">
            ...
            <div class="col-md-4">
                ...
                <div class="card border-0 shadow-sm bg-info bg-opacity-10">
                    <div class="card-body">
                        <h6 class="card-title text-info fw-bold mb-3"><i class="mdi mdi-warehouse me-1"></i> Warehouse
                            Availability</h6>
                        ...
                        <div class="d-flex align-items-baseline mb-2">
                            ...
                            <small class="text-muted ms-2">{{ $stockRequest->product->unit ?? 'Units' }}
                                available</small>
                        </div>
                        ...
                        <div class="mt-3">
                            ...
                            <ul class="list-group list-group-flush bg-transparent mt-1">
                                @forelse($stockRequest->product->batches->take(3) as $batch)
                                    <li
                                        class="list-group-item bg-transparent d-flex justify-content-between px-0 py-1 border-bottom border-info border-opacity-25">
                                        <span class="small text-dark">{{ $batch->batch_number }} <span
                                                class="text-muted">(Exp:
                                                {{ $batch->expiry_date ? $batch->expiry_date->format('M Y') : 'N/A' }})</span></span>
                                        <span class="small fw-bold">{{ $batch->quantity }}</span>
                                    </li>
                                @empty
                                    <li class="list-group-item bg-transparent text-danger small fst-italic">No active
                                        batches found!</li>
                                @endforelse
                                @if ($stockRequest->product->batches->count() > 3)
                                    <li class="list-group-item bg-transparent text-center small text-muted py-1">
                                        +{{ $stockRequest->product->batches->count() - 3 }} more batches</li>
                                @endif
                            </ul>
                        </div>
                    </div>
                        ...
                        @if ($stockRequest->pending_quantity > 0 && $stockRequest->status != 'rejected')
                                ...
                                <div class="mb-4" id="dispatch_qty_div">
                                    ...
                                    <div class="input-group input-group-lg has-validation">
                                        <input type="number" name="dispatch_quantity" id="dispatch_qty"
                                            ...
                                            min="1" required>
                                        <span
                                            class="input-group-text bg-light text-muted">{{ $stockRequest->product->unit ?? 'Units' }}</span>
                                    ...
                                    <div class="form-text text-muted mt-2">
                                        ...
                                        Max dispatchable:
                                        <strong>{{ min($stockRequest->pending_quantity, $totalStock) }}</strong>
                                    </div>
                                </div>
                            ...
                            <div id="rejectSection"
                                ...
                                <form id="rejectForm">
                                    ...
                                    <label class="form-label small text-dark">Reason for Rejection <span
                                            class="text-danger">*</span></label>
                                    ...
                                    <div class="d-flex justify-content-end gap-2">
                                        ...
                                        <button type="submit" class="btn btn-danger btn-sm px-3">Confirm
                                            Rejection</button>
                                    </div>
                                </form>
                            </div>
                        @else
                            ...
                            <div class="text-center py-5">
                                @if ($stockRequest->status == 'rejected')
                                    <div class="mb-3">
                                        <i class="mdi mdi-close-circle text-danger opacity-50"
                                ...
                                @elseif($stockRequest->status == 'dispatched')
                                    <div class="mb-3">
                                        <i class="mdi mdi-truck-check text-info opacity-50"
                                            style="font-size: 4rem;"></i>
                                    </div>
                                ...
                                @else
                                    <div class="mb-3">
                                        <i class="mdi mdi-check-decagram text-success opacity-50"
                                            style="font-size: 4rem;"></i>
                                    </div>
                                    ...
                                    <div class="mt-4 p-3 bg-light rounded text-start mx-auto"
                                        ...
                                        <div class="mb-2">
                                            ...
                                            @if ($stockRequest->store_payment_proof)
                                                <a href="{{ asset('storage/' . $stockRequest->store_payment_proof) }}"
                                        ...
                                        <div>
                                            ...
                                            @if ($stockRequest->warehouse_payment_proof)
                                                <a href="{{ asset('storage/' . $stockRequest->warehouse_payment_proof) }}"
                                                    target="_blank" class="text-primary text-decoration-none">
                                ...
                                <a href="{{ route('warehouse.stock-requests.index') }}"
                                    class="btn btn-dark mt-3 d-block mx-auto" style="width: fit-content;">Return to
                                    Dashboard</a>
                            </div>
                        @endif
                    </div>
                ...
                <div class="modal-body">
                    ...
                    <div class="mb-3">
                        ...
                        <div class="p-3 border rounded bg-light text-center">
                            ...
                            @else
                                <span class="text-danger small"><i class="mdi mdi-alert-circle me-1"></i> No payment
                                    proof uploaded by store.</span>
                            @endif
                        </div>
                    </div>
    ...
    @push('scripts')
        <script>
            ...
            // Reusable AJAX function (same as index)
