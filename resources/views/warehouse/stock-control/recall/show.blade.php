<x-app-layout title="Recall Request #{{ $recall->id }}">
    ...
    <div class="container-fluid">
                ...
                <div
                    ...
                    <div class="d-flex flex-column gap-1">
                        @include('warehouse.partials.breadcrumb', [
                            'title' => 'Recall Request #' . $recall->id,
                        ])
        ...
        <div class="row g-4">
            ...
            <div class="col-lg-4">
                <div class="card border-0 shadow-sm h-100">
                    <div class="card-header bg-white border-bottom py-3">
                        <h6 class="mb-0 fw-bold text-dark"><i class="mdi mdi-file-document-outline me-1"></i> Request
                            Details</h6>
                    </div>
                    <div class="card-body">
                        <div class="mb-4">
                            ...
                            <div class="d-flex align-items-center mt-2">
                                ...
                                <div>
                                    ...
                                    <small class="text-muted font-monospace">UPC: {{ $recall->product->upc }}</small>
                                </div>
                            </div>
                        </div>
            ...
            <div class="col-lg-8">
                <div class="card border-0 shadow-sm h-100">
                    <div class="card-header bg-white border-bottom py-3">
                        <h6 class="mb-0 fw-bold text-primary"><i class="mdi mdi-list-status me-1"></i> Workflow &
                            Actions</h6>
                    </div>
                    <div class="card-body p-4">
                        ...
                        @if ($recall->status == 'pending_warehouse_approval')
                            <div
                                class="alert alert-warning border-0 border-start border-4 border-warning shadow-sm d-flex align-items-start mb-4">
                                <i class="mdi mdi-alert-circle fs-4 me-3 mt-1"></i>
                                <div>
                                    <h6 class="fw-bold mb-1">Action Required</h6>
                                    <p class="mb-0 small">The store has requested to return this stock. Please review
                                        and approve or reject.</p>
                                </div>
                            </div>

                            <div class="bg-light p-4 rounded border">
                                <form action="{{ route('warehouse.stock-control.recall.approve', $recall->id) }}"
                                    method="POST">
                                    @csrf
                                    <div class="row g-3 mb-3">
                                        <div class="col-md-6">
                                            <label class="form-label fw-bold">Approve Quantity <span
                                                    class="text-danger">*</span></label>
                                            <div class="input-group">
                                                <input type="number" name="approved_quantity" class="form-control"
                                                    value="{{ $recall->requested_quantity }}"
                                                    max="{{ $recall->requested_quantity }}" min="1" required>
                                                <span class="input-group-text bg-white text-muted">Units</span>
                                            </div>
                                        </div>
                                        <div class="col-12">
                                            <label class="form-label fw-bold">Remarks <span
                        ...
                        @elseif($recall->status == 'dispatched')
                            <div
                                class="alert alert-info border-0 border-start border-4 border-info shadow-sm d-flex align-items-start mb-4">
                                <i class="mdi mdi-truck-fast fs-4 me-3 mt-1"></i>
                                <div>
                                    <h6 class="fw-bold mb-1">Stock Incoming</h6>
                                    <p class="mb-0 small">Store has dispatched
                                        <strong>{{ $recall->dispatched_quantity }}</strong> units. Confirm receipt once
                                        verified.</p>
                                </div>
                            </div>

                            <div class="bg-light p-4 rounded border">
                                <form action="{{ route('warehouse.stock-control.recall.receive', $recall->id) }}"
                                    method="POST">
                                    @csrf
                                    <div class="row g-3 mb-3">
                                        <div class="col-md-6">
                                            <label class="form-label fw-bold">Received Quantity <span
                                                    class="text-danger">*</span></label>
                                            <div class="input-group">
                                        ...
                                        <div class="col-12">
                                            <label class="form-label fw-bold">Receipt Remarks <span
                                                    class="text-muted fw-normal">(Optional)</span></label>
                        ...
                        @elseif($recall->status == 'approved')
                            <div class="text-center py-5">
                                <div
                                    class="avatar-lg bg-primary bg-opacity-10 rounded-circle mx-auto d-flex align-items-center justify-content-center mb-3">
                                    <i class="mdi mdi-clock-outline text-primary fs-1"></i>
                                </div>
                                <h5 class="fw-bold text-dark">Approved & Waiting</h5>
                                <p class="text-muted">You have approved this request. Waiting for the Store to dispatch
                                    the stock.</p>
                            </div>
                        @elseif($recall->status == 'pending_store_approval')
                            <div class="text-center py-5">
                                <div
                                    class="avatar-lg bg-warning bg-opacity-10 rounded-circle mx-auto d-flex align-items-center justify-content-center mb-3">
                                    <i class="mdi mdi-account-clock text-warning fs-1"></i>
                                </div>
                                <h5 class="fw-bold text-dark">Pending Store Action</h5>
                                <p class="text-muted">Request sent. Waiting for the Store Manager to review and accept.
                                </p>
                            </div>
                        @elseif($recall->status == 'completed')
                            <div class="text-center py-5">
                                <div
                                    class="avatar-lg bg-success bg-opacity-10 rounded-circle mx-auto d-flex align-items-center justify-content-center mb-3">
                                    <i class="mdi mdi-check-decagram text-success fs-1"></i>
                                </div>
                                <h5 class="fw-bold text-success">Recall Completed</h5>
                                <p class="text-muted">Stock has been successfully returned and added back to warehouse
                                    inventory.</p>
                            </div>
                        @else
                            <div class="alert alert-secondary d-flex align-items-center">
                                <i class="mdi mdi-information me-2"></i> Current Status:
                                <strong>{{ $recall->getStatusLabel() }}</strong>
                            </div>
                        @endif
    ...
    <div class="modal fade" id="rejectModal" tabindex="-1" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered">
            <form action="{{ route('warehouse.stock-control.recall.reject', $recall->id) }}" method="POST"
                class="modal-content border-0 shadow">
                @csrf
                <div class="modal-header bg-danger text-white">
                    <h5 class="modal-title fw-bold"><i class="mdi mdi-alert-circle-outline me-1"></i> Reject Request
                    </h5>
                ...
                <div class="modal-body p-4">
                    <div class="mb-3">
                        <label class="form-label fw-bold">Reason for Rejection <span
                                class="text-danger">*</span></label>
                        <textarea name="warehouse_remarks" class="form-control" rows="4" placeholder="Please provide a reason..."
                            required></textarea>
                    </div>
